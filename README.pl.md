...
Godzina to najwcześniejszy możliwy moment startu, nie gwarancja. WordPress nie ma własnego zegara; WP-Cron budzi się tylko wtedy, gdy jakieś żądanie uruchomi PHP. Uruchomienie spóźnia się, gdy:
...
Bo WordPress nie ma własnego zegara. WP-Cron, jego harmonogram, budzi się tylko wtedy, gdy jakieś żądanie uruchomi PHP na stronie - odsłona, ekran panelu, wywołanie REST. Wybrana godzina to więc najwcześniejszy możliwy moment startu: uruchomienie faktycznie rusza z pierwszym takim żądaniem po niej. Typowe powody spóźnienia:
...
- `DISABLE_WP_CRON` i cron systemowy: uruchomienie odbywa się przy najbliższym odpaleniu tego crona, co 5 minut albo co godzinę, jak skonfigurował hosting.
